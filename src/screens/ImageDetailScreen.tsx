...
type ImageDetailScreenProps = {
  ...
  route: { params?: { image?: FlickrPhoto } };
};

const { width } = Dimensions.get('window');

const ImageDetailScreen = ({ navigation, route }: ImageDetailScreenProps) => {
  ...
  const doubleTapGesture = Gesture.Tap()
    ...
    .onEnd(() => {
      'worklet';
      scale.value = withTiming(scale.value > 1 ? 1 : 2, { duration: 180 });
    });
        ...
        <View style={styles.card}>
          {image?.url_s ? (
            <GestureDetector gesture={composedGesture}>
              <Animated.View style={styles.heroClip}>
                <Animated.Image
